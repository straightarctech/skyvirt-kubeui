...
func main() {
	...
	var auditCM *audit.ConfigMapStore
	switch {
	case pgDB != nil:
		auditStore = audit.NewPostgresStore(pgDB, logger)
		logger.Info("audit: using durable postgres store")
	case kc != nil && kc.Clientset != nil:
		// Durable without an external DB: an in-memory ring flushed to a ConfigMap
		// in KubeUI's namespace, reloaded on restart. Air-gap friendly.
		auditCM = audit.NewConfigMapStore(kc.Clientset, 2000, logger)
		go auditCM.Run(context.Background())
		auditStore = auditCM
		logger.Info("audit: using durable ConfigMap store (set DATABASE_URL for Postgres)")
	default:
		auditStore = audit.NewMemoryStore(2000)
		logger.Info("audit: using in-memory store (lost on restart)")
	}
	...
	// Persist the last audit entries before exit.
	if auditCM != nil {
		auditCM.Flush(context.Background())
	}
