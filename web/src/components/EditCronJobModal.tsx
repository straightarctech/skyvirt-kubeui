...
/**
 * Edit a CronJob. Unlike a Job, a CronJob is fully mutable — schedule, suspend,
 * concurrency policy, and the jobTemplate's container image/env can all change.
 * Everything is merged into the live object so the rest of the spec is preserved.
 */
export default function EditCronJobModal({ namespace, name, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    getResourceYAML("CronJob", namespace, name)
      .then((obj) => {
        if (cancelled) return;
        setOriginal(obj);
        const spec = (obj.spec || {}) as Record<string, any>;
        setSchedule(String(spec.schedule || ""));
        setSuspend(spec.suspend === true);
        setConcurrency(String(spec.concurrencyPolicy || "Allow"));
        const cs = (spec.jobTemplate?.spec?.template?.spec?.containers || []) as Record<string, any>[];
  ...
  const setImage = (i: number, image: string) => setContainers((cs) => cs.map((c, idx) => (idx === i ? { ...c, image } : c)));
  const addEnv = (i: number) => setContainers((cs) => cs.map((c, idx) => (idx === i ? { ...c, env: [...c.env, { name: "", value: "" }] } : c)));
  const removeEnv = (i: number, j: number) => setContainers((cs) => cs.map((c, idx) => (idx === i ? { ...c, env: c.env.filter((_, k) => k !== j) } : c)));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      obj.spec = obj.spec || {};
      obj.spec.schedule = schedule.trim();
      obj.spec.suspend = suspend;
      obj.spec.concurrencyPolicy = concurrency;
      const specContainers = (obj.spec.jobTemplate?.spec?.template?.spec?.containers || []) as Record<string, any>[];
      ...
      await updateResourceYAML("CronJob", namespace, name, jsYaml.dump(obj, { noRefs: true }));
      toast.success("CronJob updated"); onSaved();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div role="dialog" aria-modal="true" className="relative bg-th-panel rounded-xl shadow-card w-full max-w-2xl mx-4 max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between border-b border-th-line px-6 py-4">
          <h3 className="text-lg font-semibold text-th-heading">Edit CronJob · <span className="text-th-dim font-normal">{namespace}/{name}</span></h3>
          ...
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-th-dim mb-1">Schedule (cron)</label>
                  <input type="text" value={schedule} onChange={(e) => setSchedule(e.target.value)} placeholder="*/5 * * * *" className={inputCls + " font-mono"} />
                </div>
                <div>
                  <label className="block text-xs text-th-dim mb-1">Concurrency Policy</label>
        ...
        <div className="flex justify-end gap-2 border-t border-th-line px-6 py-4">
          ...
          <button onClick={handleSave} disabled={saving || loading || !schedule.trim()} className="px-4 py-2 text-sm bg-th-accent text-white rounded-lg hover:opacity-90 disabled:opacity-50">
