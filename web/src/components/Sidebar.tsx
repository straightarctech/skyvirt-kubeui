...
export const navigation: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: "CI/CD", path: "/operations/cicd" },
...
export default function Sidebar({ collapsed, onToggle, mobileOpen = false, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 h-screen bg-th-sidebar backdrop-blur-xl flex flex-col z-40 transition-all duration-200 border-r border-white/[0.06] w-56 ${
      ...
      <div className="flex items-center h-14 px-4 border-b border-white/[0.06] shrink-0">
        ...
        <button
          onClick={onToggle}
          className={`text-th-sidebar-text-dim hover:text-th-sidebar-text hover:bg-white/[0.06] rounded-md p-1 transition-all ${contentCollapsed ? "mx-auto" : "ml-auto"}`}
        >
          {icons.collapse}
        </button>
      </div>
          ...
          return (
            <div key={group.label} className="mb-0.5">
              ...
              {!contentCollapsed && isExpanded && (
                <div className="ml-5 mt-0.5 space-y-0.5 animate-fade-in border-l border-white/[0.06] pl-0">
      ...
      {/* Footer */}
      {!contentCollapsed && (
        <div className="px-4 py-3 border-t border-white/[0.06] text-[10px] text-th-sidebar-text-dim flex items-center justify-between">
          <a
