...
export default function Helm() {
  ...
  // Upgrade modal — edit values, preview the rendered diff, then apply.
  const [showUpgrade, setShowUpgrade] = useState(false);
  const [upgradeForm, setUpgradeForm] = useState({ chart: "", repo_url: "", version: "", values_yaml: "" });
  const [upgrading, setUpgrading] = useState(false);
  const [upgradeError, setUpgradeError] = useState("");
  const [previewing, setPreviewing] = useState(false);
  const [previewDiff, setPreviewDiff] = useState<{ current: string; proposed: string } | null>(null);
  ...
  // Strip a trailing "-<version>" from a release's chart id ("nginx-15.1.0" →
  // "nginx") to seed the chart field; the user can refine it (e.g. "repo/chart").
  function chartName(chart: string): string {
    return chart.replace(/-v?\d[\w.\-+]*$/, "");
  }

  function openUpgrade(r: HelmRelease) {
    setSelectedRelease(r);
    setUpgradeForm({ chart: chartName(r.chart || ""), repo_url: "", version: "", values_yaml: releaseValues || "" });
    setPreviewDiff(null);
    setUpgradeError("");
    setShowUpgrade(true);
  }
  ...
  function upgradePayload() {
    return {
      chart: upgradeForm.chart.trim(),
      repo_url: upgradeForm.repo_url.trim() || undefined,
      version: upgradeForm.version.trim() || undefined,
      values_yaml: upgradeForm.values_yaml,
    };
  }
  ...
  async function handlePreview() {
    ...
    setPreviewDiff(null);
    try {
      const diff = await previewHelmUpgrade(selectedRelease.namespace, selectedRelease.name, upgradePayload());
      setPreviewDiff(diff);
    } catch (err) {
      setUpgradeError(err instanceof Error ? err.message : "Preview failed");
    } finally {
      setPreviewing(false);
    }
  }
  ...
  async function handleUpgrade() {
    ...
    setUpgradeError("");
    try {
      await upgradeHelmRelease(selectedRelease.namespace, selectedRelease.name, upgradePayload());
  ...
  return (
    <div className="space-y-4 animate-fade-in">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setSelectedRelease(null)}>
          <div className="bg-th-panel border border-th-line rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex justify-between pt-2">
              <div className="flex gap-2">
                <button
                  onClick={() => selectedRelease && openUpgrade(selectedRelease)}
                  disabled={actionLoading}
      ...
      {showUpgrade && selectedRelease && (
        <div className="fixed inset-0 z-[60] flex items-start justify-center overflow-y-auto bg-black/50 p-4 sm:p-8" onClick={() => setShowUpgrade(false)}>
          <div className="bg-th-panel border border-th-line rounded-xl shadow-2xl w-full max-w-3xl p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
            <div>
              <h2 className="text-lg font-semibold text-th-heading">Upgrade {selectedRelease.name}</h2>
              <p className="text-xs text-th-dim">Current chart: {selectedRelease.chart} (rev {selectedRelease.revision}) · preview renders the change without applying it.</p>
            </div>
            {upgradeError && <div className="p-2 bg-th-danger-s text-th-danger rounded text-sm whitespace-pre-wrap">{upgradeError}</div>}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                <label className="block text-xs font-medium text-th-label mb-1">Chart</label>
                <input className={INPUT_CLS} placeholder="repo/chart or chart" value={upgradeForm.chart} onChange={(e) => setUpgradeForm((f) => ({ ...f, chart: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-th-label mb-1">Repo URL <span className="text-th-ghost font-normal">(if needed)</span></label>
                <input className={INPUT_CLS} placeholder="https://charts…" value={upgradeForm.repo_url} onChange={(e) => setUpgradeForm((f) => ({ ...f, repo_url: e.target.value }))} />
              </div>
              <div>
                <label className="block text-xs font-medium text-th-label mb-1">Version <span className="text-th-ghost font-normal">(empty = latest)</span></label>
            ...
            <div>
              <label className="block text-xs font-medium text-th-label mb-1">Values <span className="text-th-ghost font-normal">(values.yaml)</span></label>
              <YAMLEditor value={upgradeForm.values_yaml} onChange={(v) => { setUpgradeForm((f) => ({ ...f, values_yaml: v })); setPreviewDiff(null); }} height="220px" />
            </div>
            {previewDiff && (
              <div>
                <label className="block text-xs font-medium text-th-label mb-1">Rendered change</label>
                <DiffView before={previewDiff.current} after={previewDiff.proposed} height="30vh" label="current → proposed · manifest" />
              </div>
            )}
            <div className="flex justify-between gap-2 pt-2">
              <button
                onClick={handlePreview}
                disabled={previewing || upgrading || !upgradeForm.chart.trim()}
                className="px-4 py-2 text-sm bg-th-subtle border border-th-line text-th-body rounded-lg hover:bg-th-hover disabled:opacity-50"
              >
                {previewing ? "Rendering…" : "Preview changes"}
              </button>
              <div className="flex gap-2">
                ...
                <button onClick={handleUpgrade} disabled={upgrading || previewing || !upgradeForm.chart.trim()}
                  className="px-4 py-2 text-sm bg-th-accent text-white rounded-lg hover:opacity-90 disabled:opacity-50">
