...
import { listNodes, scanDeprecatedAPIs, getUpgradeReadiness } from "@/api/client";
import type { NodeSummary, APIFinding, ReadinessReport, ReadinessCheck } from "@/api/client";
...
// Verdict → presentation. block/warn/pass map to the shared status palette.
const VERDICT: Record<string, { label: string; cls: string; dot: string }> = {
  pass: { label: "Ready to upgrade", cls: "bg-th-ok-s text-th-ok", dot: "bg-th-ok" },
  warn: { label: "Upgrade with caution", cls: "bg-th-warn-s text-th-warn", dot: "bg-th-warn" },
  block: { label: "Not ready — resolve blockers", cls: "bg-th-danger-s text-th-danger", dot: "bg-th-danger" },
};
...
function ReadinessPanel() {
  ...
  const run = async () => {
    ...
    setError("");
    try {
      setReport(await getUpgradeReadiness());
    } catch (e) {
  ...
  const checkRow = (c: ReadinessCheck) => {
    const v = VERDICT[c.status] ?? VERDICT.pass;
    return (
      <div key={c.id} className="border-t border-th-line py-3 first:border-0">
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 shrink-0 rounded-full ${v.dot}`} />
          <span className="text-sm font-medium text-th-heading">{c.title}</span>
          <span className={`ml-auto rounded px-1.5 py-0.5 text-[11px] font-semibold uppercase ${v.cls}`}>{c.status}</span>
        </div>
        <p className="mt-1 pl-4 text-xs text-th-dim">{c.summary}</p>
        {c.detail && c.detail.length > 0 && (
          <ul className="mt-1 space-y-0.5 pl-4">
            {c.detail.map((d, i) => (
              <li key={i} className="font-mono text-[11px] text-th-body">• {d}</li>
            ))}
          </ul>
        )}
      </div>
  ...
  const v = report ? (VERDICT[report.verdict] ?? VERDICT.pass) : null;

  return (
    <div className="space-y-3 rounded-xl border border-th-line bg-th-panel p-5 shadow-card">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-th-heading">Upgrade readiness — go / no-go</h2>
          <p className="mt-0.5 text-xs text-th-dim">
            One verdict from deprecated APIs, certificate expiry, node health, and disruption budgets. Read-only.
          </p>
        </div>
        <button onClick={run} disabled={running} className="rounded-lg bg-th-accent px-3 py-1.5 text-sm text-white hover:opacity-90 disabled:opacity-50">
      ...
      {report && v && (
        <>
          <div className={`flex items-center gap-2 rounded-lg px-3 py-2 ${v.cls}`}>
            <span className={`h-2.5 w-2.5 rounded-full ${v.dot}`} />
            <span className="text-sm font-semibold">{v.label}</span>
          </div>
          <div>{report.checks.map(checkRow)}</div>
        </>
      )}
    </div>
  );
    ...
    <div className="space-y-3 rounded-xl border border-th-line bg-th-panel p-5 shadow-card">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-th-dim">
            Scans Helm releases <span className="text-th-body">and live kubectl-applied objects</span> (via their last-applied config) for API versions removed in newer Kubernetes. Fix these before upgrading.
          </p>
        </div>
      ...
      {findings !== null &&
        (findings.length === 0 ? (
          <div className="flex items-center gap-2 rounded-lg bg-th-ok-s px-3 py-2 text-sm text-th-ok">
            ✓ No deprecated APIs found in Helm releases or live objects — you're clear to upgrade.
          </div>
        ) : (
          <div>
