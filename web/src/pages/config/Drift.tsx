...
function TimelineModal({ target, onClose }: { target: Ref; onClose: () => void }) {
  const { data, loading, refresh } = useResource<DriftSnapshot[]>(() => getDriftTimeline(target.kind, target.namespace, target.name), [target.kind, target.namespace, target.name]);
  const toast = useToast();
  ...
  // Roll the resource back to an earlier snapshot. A pre-revert snapshot is taken
  // first (best-effort) so the revert is itself undoable; the apply is the same
  // server-side path as the live drift revert.
  const revertTo = async (snap: DriftSnapshot) => {
    if (!snap.content) return;
    setReverting(true);
    try {
      await snapshotNamespace(target.namespace).catch(() => {}); // pre-revert capture
      await applyManifest(snap.content, { force: true }); // revert intentionally takes ownership
      toast.success(`Reverted ${target.kind} ${target.name} to ${when(snap.taken)}`);
      refresh();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Revert failed");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4 sm:p-8" role="dialog" aria-modal="true" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-5xl rounded-xl border border-th-line bg-th-panel shadow-xl">
        <div className="flex items-center justify-between border-b border-th-line px-5 py-3">
          ...
          <div className="flex items-center gap-2">
            {pair.length === 2 && (
              <button onClick={() => revertTo(pair[0])} disabled={reverting} title="Roll the resource back to the earlier of the two selected points" className="rounded-lg bg-th-warn-s px-3 py-1.5 text-sm text-th-warn hover:opacity-80 disabled:opacity-50">
                {reverting ? "Reverting…" : `⤺ Revert to ${when(pair[0].taken)}`}
              </button>
            )}
              ...
              snaps.map((s) => {
                ...
                return (
                  <button key={s.id} onClick={() => toggle(s.id)} className={`block w-full rounded-lg border px-3 py-2 text-left text-xs transition-colors ${on ? "border-th-accent bg-th-accent/10" : "border-th-line bg-th-subtle hover:bg-th-hover"}`}>
                    ...
                    {s.origin === "external" ? (
                      <div className="mt-0.5 font-medium text-th-warn">⚠ external change (not via KubeUI)</div>
                    ) : s.origin === "kubeui" ? (
                      <div className="mt-0.5 text-th-ok">by {s.changed_by || "?"}{s.changed_via ? ` · ${s.changed_via}` : ""}</div>
                    ) : s.origin === "initial" ? (
                      <div className="mt-0.5 text-th-ghost">initial capture</div>
                    ) : null}
                  </button>
                );
              })
