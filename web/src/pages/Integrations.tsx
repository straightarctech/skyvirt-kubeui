...
const SIGNALS: { key: keyof ForwardConfig["signals"]; label: string; hint: string }[] = [
  ...
  { key: "audit", label: "Audit log", hint: "who did what in KubeUI (scheduled — streamed once each)" },
];
...
function SiemForwarder() {
  ...
  const [signals, setSignals] = useState<ForwardConfig["signals"]>({ vuln: true, cert: true, config: false, rbac: false, audit: false });
