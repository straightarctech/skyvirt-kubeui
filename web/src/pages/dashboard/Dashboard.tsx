...
import {
  ...
  getSecurityPosture, listRiskyBindings, scanCertExpiry, listVulnReports, vulnStatus,
} from "@/api/client";
import type {
  ...
  SecurityPosture, RiskyBinding, CertInfo, VulnReport,
} from "@/api/client";
...
function MiniStat({ icon, value, label, color = "text-th-accent", to }: {
  ...
  const inner = (
    <div className="flex items-center gap-3 h-full glass-card glass-card-hover rounded-xl px-4 py-3 transition-all duration-200 group">
...
// ---------------------------------------------------------------------------
// Security tile — one security-posture metric, tone-colored, links to its page
// ---------------------------------------------------------------------------

type SecTone = "danger" | "warn" | "ok" | "accent";

function SecTile({ label, value, suffix, tone, to, hint }: {
  label: string; value: number | string; suffix?: string; tone: SecTone; to?: string; hint?: string;
}) {
  const text = tone === "danger" ? "text-th-danger" : tone === "warn" ? "text-th-warn" : tone === "ok" ? "text-th-ok" : "text-th-accent";
  const bg = tone === "danger" ? "bg-th-danger-s" : tone === "warn" ? "bg-th-warn-s" : tone === "ok" ? "bg-th-ok-s" : "bg-th-subtle";
  const inner = (
    <div className={`rounded-lg ${bg} px-3 py-4 h-full flex flex-col items-center justify-center text-center transition-all duration-200 hover:brightness-[1.03]`}>
      <p className={`text-2xl font-black leading-none ${text}`}>
        {value}{suffix && <span className="text-sm font-bold">{suffix}</span>}
      </p>
      <p className="mt-1.5 text-[10px] text-th-dim uppercase tracking-wide font-medium">{label}</p>
      {hint && <p className="mt-0.5 text-[9px] text-th-ghost">{hint}</p>}
    </div>
  );
...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async () => {
    const results = await Promise.allSettled([
      ...
      listSecrets(),
      topPods(),
      // --- security (optional; excluded from the failed-source count below) ---
      getSecurityPosture(),
      listVulnReports(),
      listRiskyBindings(),
      scanCertExpiry(),
    ...
    // Track failures so we don't render empty-as-healthy. Only the 16 core
    // sources count — the security scans are optional (e.g. Trivy may be absent),
    // so a rejection there must not raise the "data sources failed" banner.
    setFailedSources(results.slice(0, 16).filter((r) => r.status === "rejected").length);
    ...
    setData({
      ...
      secrets: get(14, []),
      podMetrics: get(15, []),
      securityPosture: get<SecurityPosture | null>(16, null),
      vulnReports: get(17, []),
      riskyBindings: get(18, []),
      certs: get(19, []),
      vulnInstalled: get(20, { installed: false }).installed,
    });
  ...
  const critCVEs = data.vulnReports.reduce((s, v) => s + v.critical, 0);
  const highCVEs = data.vulnReports.reduce((s, v) => s + v.high, 0);
  const riskyCount = data.riskyBindings.length;
  const expiringCerts = data.certs.filter((c) => !c.no_expiry && (c.expired || c.days_left <= 30)).length;
  const postureScore = data.securityPosture?.score ?? null;
  const postureIssues = (data.securityPosture?.counts?.high ?? 0) + (data.securityPosture?.counts?.medium ?? 0);
  const postureTone: SecTone = postureScore === null ? "accent" : postureScore >= 80 ? "ok" : postureScore >= 50 ? "warn" : "danger";
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        {/* Quick stats grid */}
        <div className="col-span-12 lg:col-span-3 grid grid-cols-2 grid-rows-2 gap-3 h-full">
          <MiniStat
      ...
      {/* ------------------------------------------------------------------ */}
      {/* SECURITY POSTURE (security-first — the console's headline)          */}
      {/* ------------------------------------------------------------------ */}
      <div className="glass-card rounded-xl p-4">
        <SectionHeader title="Security Posture" to="/observability/security" />
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <SecTile
            label="Posture Score" value={postureScore === null ? "—" : postureScore}
            suffix={postureScore === null ? undefined : "/100"} tone={postureTone}
            to="/observability/security"
          />
          <SecTile
            label="Critical CVEs" value={data.vulnInstalled ? critCVEs : "—"}
            tone={critCVEs > 0 ? "danger" : "ok"} to="/observability/vulnerabilities"
            hint={data.vulnInstalled ? undefined : "Trivy off"}
          />
          <SecTile
            label="High CVEs" value={data.vulnInstalled ? highCVEs : "—"}
            tone={highCVEs > 0 ? "warn" : "ok"} to="/observability/vulnerabilities"
            hint={data.vulnInstalled ? undefined : "install Trivy"}
          />
          <SecTile
            label="Risky RBAC" value={riskyCount}
            tone={riskyCount > 0 ? "warn" : "ok"} to="/config/access-review"
          />
          <SecTile
            label="Certs ≤30d" value={expiringCerts}
            tone={expiringCerts > 0 ? "warn" : "ok"} to="/observability/certificates"
          />
          <SecTile
            label="Posture Issues" value={postureIssues}
            tone={postureIssues > 0 ? "warn" : "ok"} to="/observability/security"
          />
        </div>
      </div>
