...
export default function Certificates() {
  ...
  const sorted = useMemo(() => [...(data ?? [])].sort((a, b) => a.days_left - b.days_left), [data]);
  const stats = useMemo(() => {
    const list = data ?? [];
    return {
      total: list.length,
      expired: list.filter((c) => c.expired).length,
      soon: list.filter((c) => !c.expired && c.days_left <= 30).length,
    };
  }, [data]);
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-th-heading">Certificate Expiry</h1>
          <p className="mt-0.5 text-sm text-th-dim">
            TLS certificates across your Secrets and when they expire — catch a silent expiry before it takes a service down.
          </p>
        </div>
      ...
      {!loading && (data?.length ?? 0) > 0 && (
        <StatStrip
          stats={[
            { label: "Certificates", value: stats.total, tone: "accent" },
      ...
      {loading ? (
        ...
        <EmptyState title="No TLS certificates found" hint="No Secrets carrying a tls.crt/ca.crt were found in the cluster." />
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-th-subtle text-left text-xs text-th-ghost">
              <tr>
                ...
                <th className="px-4 py-2">Common name / SANs</th>
              ...
              {sorted.map((c, i) => {
                ...
                return (
                  <tr key={i} className="hover:bg-th-hover">
                    ...
                    <td className="px-4 py-2 text-xs text-th-dim">{new Date(c.not_after).toLocaleDateString()}</td>
