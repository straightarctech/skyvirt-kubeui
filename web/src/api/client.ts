...
// getDrainPlan returns a dry-run drain plan (no cluster change): what would be
// evicted, what is exempt, and which pods a PodDisruptionBudget guards.
export async function getDrainPlan(name: string): Promise<DrainReport> {
  return request(`/nodes/${name}/drain-plan`);
}

export async function drainNode(name: string, opts?: { grace_period_seconds?: number; timeout_seconds?: number }): Promise<DrainReport> {
  return request(`/nodes/${name}/drain`, { method: "POST", body: JSON.stringify(opts ?? {}) });
}
...
export interface HelmUpgradeData {
  repo_url?: string;
  chart?: string;
  version?: string;
  values?: Record<string, string>;
...
export async function upgradeHelmRelease(namespace: string, name: string, data: HelmUpgradeData): Promise<{ message: string; output: string }> {
  return request(`/namespaces/${namespace}/helm/releases/${name}`, {
    method: "PUT",
...
// previewHelmUpgrade renders the proposed upgrade without applying it, returning
// the current and proposed manifests to diff before committing.
export async function previewHelmUpgrade(namespace: string, name: string, data: HelmUpgradeData): Promise<{ current: string; proposed: string }> {
  return request(`/namespaces/${namespace}/helm/releases/${name}/upgrade/preview`, {
    method: "POST",
    body: JSON.stringify(data),
  });
}
...
// getUpgradeReadiness returns the pre-upgrade go/no-go verdict, rolled up from
// the deprecated-API scan, cert expiry, node health, and disruption budgets.
export async function getUpgradeReadiness(): Promise<ReadinessReport> {
  return request("/upgrade/readiness");
}
...
export interface CertInfo {
  ...
  days_left: number;
  expired: boolean;
  kind: "tls" | "kubeconfig" | "sa-token";
  no_expiry?: boolean; // long-lived credential (e.g. a legacy SA token) — no expiry
}
...
export interface ExposureItem {
  ...
  type: string; namespace: string; name: string;
  address?: string; ports?: string; tls?: boolean; detail?: string;
}
export interface ExposureReport {
  items: ExposureItem[];
  counts: Record<string, number>;
  load_balancers: number; node_ports: number; ingresses: number; plaintext_ingresses: number;
}
// getExposure: the cluster's external attack surface (LoadBalancer/NodePort
// services + Ingress routes, plaintext ones flagged).
export async function getExposure(): Promise<ExposureReport> {
  return request("/security/exposure");
}

export interface UnrestrictedWorkload { namespace: string; kind: string; name: string; }
export interface NetworkPolicyCoverageReport {
  unrestricted: UnrestrictedWorkload[];
  total_workloads: number;
  covered_workloads: number;
}
// getNetpolCoverage: workloads with no ingress NetworkPolicy (open to lateral
// movement) — the internal complement to external exposure.
export async function getNetpolCoverage(): Promise<NetworkPolicyCoverageReport> {
  return request("/security/netpol-coverage");
}

export interface HealthIssue {
  severity: "critical" | "high" | "medium";
  category: string; kind: string; namespace?: string; name: string;
...
// getClusterHealth: whole-cluster triage — everything broken or stuck right now.
export async function getClusterHealth(): Promise<ClusterHealthReport> {
  return request("/cluster/health");
}

export interface OrphanItem {
  severity: "medium" | "low"; category: string; kind: string;
  namespace: string; name: string; detail: string;
}
export interface OrphanReport {
  items: OrphanItem[]; dead_services: number; unused_pvcs: number; unused_storage?: string;
}
// getOrphans: broken/unused resources to review (dead services, unused PVCs).
export async function getOrphans(): Promise<OrphanReport> {
  return request("/cluster/orphans");
}

export interface ImageUse {
  image: string; registry: string; repository: string; tag: string;
  digest?: string; mutable: boolean; pull_always: boolean; workloads: string[];
}
export interface ImageAuditReport {
  images: ImageUse[]; total: number; mutable: number; registries: Record<string, number>;
}
// listImageAudit: image-provenance inventory (registries, tags, mutable refs).
export async function listImageAudit(): Promise<ImageAuditReport> {
  return request("/security/image-audit");
}

export interface SecFinding { check: string; severity: "critical" | "high" | "medium" | "low"; detail: string; }
export interface WorkloadRisk {
  namespace: string; kind: string; name: string;
  severity: "critical" | "high" | "medium" | "low"; findings: SecFinding[];
}
// listWorkloadAudit: insecure workload settings (privileged, host namespaces,
// hostPath, root, added caps) per workload. No scanner required.
export async function listWorkloadAudit(): Promise<WorkloadRisk[]> {
  return request("/security/workload-audit");
}
...
export interface ForwardSignals { vuln: boolean; cert: boolean; config: boolean; rbac: boolean; audit: boolean; }
...
export interface AdminPath { binding: string; scope: string; role: string; via: string; }
export interface AdminReachSubject {
  kind: string; name: string; namespace?: string;
  severity: "critical" | "high"; paths: AdminPath[];
}
// listAdminReach: subjects that can reach cluster-admin (directly or by
// self-escalation), each with the path.
export async function listAdminReach(): Promise<AdminReachSubject[]> {
  return request("/rbac/admin-reach");
}
...
export async function applyManifest(manifest: string, opts?: { force?: boolean }): Promise<ApplyResult[]> {
  return request("/apply", {
    method: "POST",
    body: JSON.stringify({ manifest, force: opts?.force ?? false }),
  });
}

/** GitOps-lite: clone a repo and return the manifests under a path. */
// ---------------------------------------------------------------------------
// Drift timeline (snapshot history)
// ---------------------------------------------------------------------------
...
export interface DriftSnapshot {
  ...
  source: string;
  content?: string;
  origin?: "kubeui" | "external" | "initial";
  changed_by?: string;
  changed_via?: string;
}
export interface TrackedRef { kind: string; namespace: string; name: string; count: number; latest: string; }

export async function getDriftTimeline(kind: string, namespace: string, name: string): Promise<DriftSnapshot[]> {
  const q = `kind=${encodeURIComponent(kind)}&namespace=${encodeURIComponent(namespace)}&name=${encodeURIComponent(name)}`;
  return request<DriftSnapshot[]>(`/drift/timeline?${q}`);
}
export async function getTrackedDrift(): Promise<TrackedRef[]> {
  return request<TrackedRef[]>("/drift/tracked");
}
export async function snapshotNamespace(namespace: string): Promise<{ new: number }> {
  return request("/drift/snapshot", { method: "POST", body: JSON.stringify({ namespace }) });
}
...
// GitOps auto-sync sources: tracked repos reconciled on a schedule.
...
export async function listGitOpsSources(): Promise<GitOpsSource[]> {
  return request<GitOpsSource[]>("/gitops/sources");
}
export async function saveGitOpsSource(s: Partial<GitOpsSource>): Promise<GitOpsSource> {
  return request("/gitops/sources", { method: "POST", body: JSON.stringify(s) });
}
export async function deleteGitOpsSource(id: string): Promise<void> {
  await request(`/gitops/sources/${id}`, { method: "DELETE" });
}
export async function syncGitOpsSource(id: string): Promise<GitOpsSource> {
  return request(`/gitops/sources/${id}/sync`, { method: "POST" });
}
