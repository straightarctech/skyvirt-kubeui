...
func (h *HelmHandler) Upgrade(w http.ResponseWriter, r *http.Request) {
	...
	out, err := h.kc.HelmUpgrade(r.Context(), ns, name, req.RepoURL, req.Chart, req.Version, req.Values, req.ValuesYAML, false)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
...
// UpgradePreview renders the proposed upgrade (edited values / new version)
// without applying it, returning the current and proposed manifests so the UI
// can show a diff before the user commits.
func (h *HelmHandler) UpgradePreview(w http.ResponseWriter, r *http.Request) {
	ns := chi.URLParam(r, "namespace")
	name := chi.URLParam(r, "name")
	...
	current, proposed, err := h.kc.HelmUpgradePreview(r.Context(), ns, name, req.RepoURL, req.Chart, req.Version, req.Values, req.ValuesYAML)
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
	writeJSON(w, http.StatusOK, map[string]string{"current": current, "proposed": proposed})
}
