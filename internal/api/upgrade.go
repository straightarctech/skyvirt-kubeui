...
// Readiness returns the pre-upgrade go/no-go report — a single verdict rolled up
// from the deprecated-API scan, certificate expiry, node health, and disruption
// budgets. Entirely read-only.
func (h *UpgradeHandler) Readiness(w http.ResponseWriter, r *http.Request) {
	rep, err := h.kc.UpgradeReadiness(r.Context())
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
