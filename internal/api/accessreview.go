...
// AdminReach returns every subject that can reach cluster-admin — directly or by
// self-escalation — with the path for each.
func (h *AccessReviewHandler) AdminReach(w http.ResponseWriter, r *http.Request) {
	reach, err := h.kc.AdminReach(r.Context())
	if err != nil {
		writeError(w, http.StatusInternalServerError, err.Error())
		return
	}
