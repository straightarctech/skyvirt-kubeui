...
func NewRouter(kc *k8s.Client, pgDB *db.PostgresDB, auditStore audit.Store, webFS fs.FS, logger *zap.Logger, engineURL string, authCfg auth.Config, authzCfg AuthzConfig, aiCfg ai.Config) http.Handler {
	...
	git := gitopsHandler(kc, logger)
	go git.RunScheduler(context.Background())
	vuln := vulnHandler(kc)
	fwd := forwardHandler(kc, logger, auditStore)
	go fwd.RunScheduler(context.Background())

	// Drift timeline: ConfigMap-backed snapshot store + periodic snapshotter
	// (acts only on namespaces labelled kubeui.io/track-drift=true).
	snapStore := snapshot.NewConfigMapStore(kc.Clientset, ownNamespace())
	snapshotter := snapshot.NewSnapshotter(kc.Clientset, kc.DynamicClient, snapStore, logger)
	go snapshotter.Run(context.Background())
	drift := driftHandler(snapStore, snapshotter, auditStore)
	...
	r.Route("/api/v1", func(api chi.Router) {
		...
		api.Get("/cluster/health", clusterHealthHandler(kc))
		api.Get("/cluster/orphans", orphansHandler(kc))
		api.Get("/security/exposure", exposureHandler(kc))
		api.Get("/security/netpol-coverage", netpolCoverageHandler(kc))
		...
		api.Get("/nodes/{name}/drain-plan", nodes.DrainPlan)
		...
		api.Post("/namespaces/{namespace}/helm/releases/{name}/upgrade/preview", hlm.UpgradePreview)
		...
		api.Get("/gitops/sources", git.ListSources)
		api.Post("/gitops/sources", git.CreateSource)
		api.Delete("/gitops/sources/{id}", git.DeleteSource)
		api.Post("/gitops/sources/{id}/sync", git.SyncSource)
		api.Get("/vuln/status", vuln.Status)
		api.Get("/vuln/reports", vuln.Reports)
		api.Get("/security/workload-audit", vuln.WorkloadAudit)
		api.Get("/security/image-audit", vuln.ImageAudit)
		api.Post("/forward/security", fwd.Security)
		api.Get("/forward/config", fwd.GetConfig)
		api.Put("/forward/config", fwd.PutConfig)

		// --- Drift timeline ---
		api.Get("/drift/timeline", drift.Timeline)
		api.Get("/drift/tracked", drift.Tracked)
		api.Post("/drift/snapshot", drift.SnapshotNow)
