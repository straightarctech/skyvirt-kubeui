...
// snapshot returns a copy of the full ring (oldest→newest) and the next id — used
// by the durable ConfigMap store to persist the buffer.
func (m *MemoryStore) snapshot() ([]Entry, int64) {
	m.mu.RLock()
	defer m.mu.RUnlock()
...
// restore seeds the ring from persisted entries (oldest→newest), capped.
func (m *MemoryStore) restore(entries []Entry, nextID int64) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if len(entries) > m.cap {
		entries = entries[len(entries)-m.cap:]
	}
	m.entries = entries
	m.nextID = nextID
}
