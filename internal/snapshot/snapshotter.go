...
func NewSnapshotter(cs kubernetes.Interface, dyn dynamic.Interface, store Store, logger *zap.Logger) *Snapshotter {
	return &Snapshotter{cs: cs, dyn: dyn, store: store, logger: logger, interval: 30 * time.Minute}
}

// Run is the background loop — snapshots the tracked namespaces on an interval.
func (s *Snapshotter) Run(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-t.C:
			if n, err := s.snapshotTracked(ctx); err == nil && n > 0 && s.logger != nil {
...
func (s *Snapshotter) snapshotTracked(ctx context.Context) (int, error) {
	...
	for i := range nss.Items {
		n, _ := s.SnapshotNamespace(ctx, nss.Items[i].Name, "periodic")
		total += n
	}
	return total, nil
}
